...
def mnistnonIID(dataset,num_users,test):#function divides dataset into classes and each client gets random 3 classes to train on
    ...
    for i in range(num_users):
        np.random.seed(i)
        #print(classes_indx)
        temp=set(np.random.choice(classes_indx,4,replace=False)) #random 4 classes to each client
...
def load_dataset(num_users,iidtype):#this function helps load the datasets we made using mnistIID
    ...
    #trainset=trainset.values/255
    #testset=testset.values/255
#     sub = list(range(0, len(trainset), 6))
#     trainset = torch.utils.data.Subset(trainset, sub)
#     trainset = torch.utils.data.random_split(trainset, [10000, len(trainset)-10000])[0]
#     testset = torch.utils.data.random_split(testset, [10000, len(testset)-10000])[0]
    train_group=None
    test_group=None
    if iidtype=='iid':
